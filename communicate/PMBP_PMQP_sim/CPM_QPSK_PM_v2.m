...
fc = 1*K;                   % 主载频
fcsub = 384*K;              % 副载频
fs = 10 * fcsub;            % 采样率 -- 决定副载波能否估计准确
time = 5;                   % 仿真时间
ml =  2;                    % QPSK 调制
bit_rate = Rb * ml;
symbolnum = Rb*time;        % 仿真码元个数
bitnum = bit_rate * time;   % 仿真比特个数

SNR = 25;

%% CPM_QPSK_PM调制
% PCM编码
pcm_bits = randi([0 1],1,bitnum);

% QPSK 调制 
qpsk = zeros(1,symbolnum);
for i = 1 : symbolnum
    if pcm_bits(1+ ml*(i-1) : ml*i) == [0 0]
        qpsk(i) = 1 + 1i * 1 ;
    elseif pcm_bits(1+ ml*(i-1) : ml*i) == [0 1]
        qpsk(i) = -1 + 1i * 1 ;
    elseif pcm_bits(1+ ml*(i-1) : ml*i) == [1 1]
        qpsk(i) = -1 - 1i * 1 ;
    elseif pcm_bits(1+ ml*(i-1) : ml*i) == [1 0]
        qpsk(i) = 1 - 1i * 1 ;
    end
end
qpsk = qpsk / sqrt(2);  % 功率归一化
...
rolloff = 1;                            % 滚降系数
span = 10;                              % 滤波器长度（符号数）
sps = ceil(fs / Rb); % 每符号采样数
rrc_filter = rcosdesign(rolloff, span, sps, 'sqrt'); % 生成平方根升余弦滤波器
for k = 1 : length(qpsk)
    up_pcm_s(1+sps*(k-1)) = qpsk(k);
    up_pcm_s(2+sps*(k-1):sps*k) = zeros(1,sps-1);
end
rcos_qpsk_i = filter(rrc_filter,1,real(up_pcm_s));
rcos_qpsk_q = filter(rrc_filter,1,imag(up_pcm_s));

rcos_qpsk_s = rcos_qpsk_i + 1i* rcos_qpsk_q;
N = length(rcos_qpsk_s);
t = (1 : N)/fs;
s_QPSK = rcos_qpsk_s .* exp(1i * 2 * pi * fcsub * t);
% PM 调制
Kp = 0.8;
s_PCM_QPSK_PM = cos(2 * pi * fc * t + Kp * real(s_QPSK));
s_PCM_QPSK_PM1 = sin(2 * pi * fc * t + Kp * real(s_QPSK));      % 一个正弦一个余弦，此步必须

%% 生成方式选择
send_signal = s_PCM_QPSK_PM + 1i * s_PCM_QPSK_PM1;
% figure;plot(t, real(send_signal));title('PCM-BPSK-PM信号');xlabel('时间 (s)');ylabel('幅度');

%% 信道
% 加噪
send_signal = awgn(send_signal, SNR, 'measured');
% 加频偏
% figure;plot(abs(fft(send_signal,fs)));title("接收信号频谱");
...
PLL_Phase_hilbert = hilbert(PLL_Phase_Part_1);       % 相位进行希尔伯特变换，此步必须
%% ******** QPSK解调 ********
cpm_bpsk = PLL_Phase_hilbert;
% 采用M倍频时，
% nfft 长度取决于fc_est和fs,即满足 fc_est*M*fs/nfft < (nfft/2),才能估计出载频
% 简单得说：估计得参数要小于fs/2/M，这个M一定不能忽略
% 凡是利用谱线估计，可能都要注意这个关系
% 如果不注意这个关系，谱线就会移到右半部分（nfft/2--nfft）
...
figure;plot(fft_temp);title("载频估计频谱图")
fft_temp(1:10) = 0;
...
%% 解调
deqpsksymbol = length(I_PLL_D1);
deqpsk = zeros(1,deqpsksymbol*ml);
for i = 1 : deqpsksymbol
    if I_PLL_D1(i)>0 && Q_PLL_D1(i)>0
        deqpsk(1+ ml*(i-1) : ml*i) = [0 0];
    elseif I_PLL_D1(i)<0 && Q_PLL_D1(i)>0
        deqpsk(1+ ml*(i-1) : ml*i) = [0 1];
    elseif I_PLL_D1(i)<0 && Q_PLL_D1(i)<0
        deqpsk(1+ ml*(i-1) : ml*i) = [1 1];
    elseif  I_PLL_D1(i)>0 && Q_PLL_D1(i)<0
        deqpsk(1+ ml*(i-1) : ml*i) = [1 0];
    end
end
