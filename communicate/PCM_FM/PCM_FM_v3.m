...
samplenum_len =  min(n_temp,samplenum);         % 保护，防止报错


% 计算I(nTs)和Q(nTs)
I = zeros(1,samplenum_len);
Q = zeros(1,samplenum_len);

for i = 1 : samplenum_len
...
%% 接收信号截断处理
% estimation_len = min(16384*4,samplenum_len);   % 保护，防止报错
estimation_len = samplenum_len;
%% 载频估计
...
Discriminator_Out=zeros(1,Simulation_Length_1);
...
for i=2:Simulation_Length_1
    ...
    Discriminator_Out(i) =atan2(Q_PLL_1(i),I_PLL_1(i));
    PLL_Phase_Part_1(i)=Discriminator_Out(i)*K1(i);
    PLL_Freq_Part_1(i)=Discriminator_Out(i)*K2(i)+PLL_Freq_Part_1(i-1);
...
figure;plot(Discriminator_Out);title("鉴相输出");

Discriminator_Out_temp = Discriminator_Out*fs;
len_temp = length(Discriminator_Out_temp);

for i = len_temp:-1:2
    de_phi1(i-1) = Discriminator_Out_temp(i)-Discriminator_Out_temp(i-1);          % 前向
    de_phi2(len_temp-i+1) = Discriminator_Out_temp(i)-Discriminator_Out_temp(i-1); % 后向
end
figure;plot(de_phi1);title("成型之后信息波形");
figure;plot(abs(fft(abs(de_phi2(1:end)))));title("成型之后信息波形频谱");

%% 码速率估计
estrbsignallen =  min( 16384*2,length(de_phi2) );  % 保护机制
estrbsignal = de_phi2( 1:estrbsignallen );
...
%% 滤波操作
start_len = 5e3;           % 解调数据起始点 
de_phi1_temp = de_phi1(start_len:end); 
rcos_de_phi2 = filter(rrc_filter,1,de_phi1_temp);

% 求均值 归一化到0附近
meanB = floor(length(rcos_de_phi2)*0.7);
meanE = floor(length(rcos_de_phi2)*0.8);
meanvalue = mean( rcos_de_phi2(meanB:meanE));
rcos_de_phi2 = rcos_de_phi2 - meanvalue;

%% 降采样
send_signal = rcos_de_phi2(1:estrbsignallen);
sps = 4;
fs_orignal = fs;
fs = sps*Rb_est;
...
c1=(2*sigma*Wn1*T_nco1)/(v_k1);      %环路滤波器系数c1
c2=((T_nco1*Wn1)^2)/(v_k1);          %环路滤波器系数c2
ns=length(aI)-2;
while(i<ns)
    n_temp(i+1)=n(i)-w(ms);
...
% 起始点检测
fft_len = 16384;
signal_fft = fft(input,fft_len);
signal_fft2 = fft(database,fft_len);
xcoor_fft = fft(signal_fft.*conj(signal_fft2));
xcoor_fft = abs(xcoor_fft);
correlation_loc = find(xcoor_fft==max(xcoor_fft));
database1 = pcm_symbols(correlation_loc:end);

[error_rate,totalbit] = errorbit_compute(input,database1);
